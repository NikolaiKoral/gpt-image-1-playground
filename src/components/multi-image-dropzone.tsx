...
import { Upload, X, Image as ImageIcon, AlertCircle, Move, Plus } from 'lucide-react';
...
export function MultiImageDropZone({
    ...
    const { isDragActive, isDragAccept, isDragReject, getRootProps, getInputProps } = useDragDrop({
        onDrop: handleDrop,
        accept: ['image/*'],
        maxFiles: maxImages,
        maxSize: maxFileSize,
            ...
            {images.length > 0 && (
                <div className='space-y-2'>
                    <div className='flex items-center justify-between'>
                        ...
                        {!canAddMoreImages && (
                            <p className='text-muted-foreground text-xs'>Maksimum antal billeder nået</p>
                        )}
                    </div>
                    ...
                    <div className='grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4'>
                        {images.map((imageFile, index) => (
                            <Card
                                key={imageFile.id}
                                className={cn(
                                    'group relative cursor-move overflow-hidden transition-all duration-200',
                                    'hover:scale-105 hover:shadow-md',
                                    {
                                        'ring-primary ring-2 ring-offset-2': draggedIndex === index,
                                ...
                                <div className='relative aspect-square'>
                                    <Image
                                        ...
                                        onError={(e) => {
                                            console.error(
                                                `Failed to load image preview for ${imageFile.file.name}:`,
                                                e
                                            );
                                    ...
                                    <div className='absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 transition-opacity duration-200 group-hover:opacity-100'>
                                        <div className='flex space-x-2'>
                                            ...
                                            <div className='bg-background/90 flex h-8 w-8 items-center justify-center rounded text-xs font-medium'>
                                ...
                                <div className='space-y-1 p-3'>
                                    <p className='truncate text-xs font-medium' title={imageFile.file.name}>
                                        {imageFile.file.name}
                                    </p>
                                    <p className='text-muted-foreground text-xs'>
                        ...
                        {canAddMoreImages && (
                            <Card
                                className='border-muted-foreground/25 hover:border-primary/50 hover:bg-primary/5 aspect-square cursor-pointer border-2 border-dashed transition-colors duration-200'
                                {...getRootProps()}>
                                <input {...getInputProps()} />
                                <div className='text-muted-foreground flex h-full flex-col items-center justify-center'>
                                    <Plus className='mb-2 h-8 w-8' />
                                    <p className='text-center text-xs'>Tilføj flere</p>
                                </div>
                            </Card>
                        )}
                    </div>
