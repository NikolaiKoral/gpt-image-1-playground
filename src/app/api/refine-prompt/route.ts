...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!prompt || typeof prompt !== 'string') {
            return NextResponse.json({ error: 'Prompt er påkrævet' }, { status: 400 });
        }
...
${
    imageData
        ? `Når du analyserer billedet, fokuser på:
...
Svar kun med den forbedrede prompt på dansk - ingen forklaringer:`;
        ...
        return NextResponse.json({ error: 'Der opstod en fejl ved forbedring af prompt' }, { status: 500 });
    }
}
